# Robustness check: gravity is generated from a density field with added small-scale,
# spatially-correlated heterogeneity that the resistivity truth/MT solver
# never sees. Tests degradation under model misspecification, not measurement
# noise.
#
...
#
# Run with:  julia --project=. examples/robustness_misspecified_gravity.jl
...
#   SMARTPRIOR_VFSA_SEED, SMARTPRIOR_GRAV_SEED, SMARTPRIOR_HETERO_SEED
...
const WORK = get(ENV, "SMARTPRIOR_WORK", mktempdir(; prefix = "smartprior_hetero_"))
...
# Independent of MT / train / VFSA / gravity-noise seeds and of the slab geometry.
const HETERO_SEED = parse(Int, get(ENV, "SMARTPRIOR_HETERO_SEED", "7919"))
mkpath(WORK)
@info "working directory" WORK
@info "seeds" MT_SEED TRAIN_SEED VFSA_SEED GRAV_SEED HETERO_SEED
open(joinpath(WORK, "seeds.txt"), "w") do io
    ...
    println(io, "hetero_seed\t", HETERO_SEED)
end

# Host–slab density contrast (kg/m³). Heterogeneity std is a fraction of this.
const SLAB_DENSITY = 350.0
const HETERO_STD_FRAC = 0.075
const HETERO_SIGMA_CELLS = 2.5

# Unperturbed compare_prior_2d.jl, reference=0.1 (tmp_calib_v2/ref_0.1).
const REF_RMSE = 0.5471182163149414
const REF_CORR = 0.43822595339418496

# Index-space separable Gaussian blur; axes of length 1 are skipped.
function _gauss_kernel1d(sigma_cells::Float64)
    radius = max(1, ceil(Int, 3 * sigma_cells))
    xs = -radius:radius
    k = exp.(-0.5 .* (Float64.(xs) ./ sigma_cells) .^ 2)
    return k ./ sum(k)
end

function _blur_axis(a::Array{Float64,3}, kernel::Vector{Float64}, dim::Int)
    r = (length(kernel) - 1) ÷ 2
    nx, ny, nz = size(a)
    n = (nx, ny, nz)[dim]
    n <= 1 && return a
...
function correlated_heterogeneity(dims::NTuple{3,Int};
                                  rng::AbstractRNG,
                                  std_kg::Real,
                                  sigma_cells::Real)
    field = randn(rng, dims)
    kernel = _gauss_kernel1d(Float64(sigma_cells))
    field = _blur_axis(field, kernel, 1)
    field = _blur_axis(field, kernel, 2)
    field = _blur_axis(field, kernel, 3)
    field .-= mean(field)
    s = std(field)
    s > 0 || return zeros(dims)
    field .*= Float64(std_kg) / s
    return field
end
...
truth = add_dipping_slab(truth_halfspace(grid; log_rho = HOST_LOG_RHO);
                         ...
                         log_rho = SLAB_LOG_RHO, density = SLAB_DENSITY,
                         strike = :x)
...
# Heterogeneity lives on a copy. Resistivity truth, MT sites, and the NB
# baseline still see the unperturbed density (which they never used anyway).
hetero = correlated_heterogeneity(size(truth.density);
                                  rng = Xoshiro(HETERO_SEED),
                                  std_kg = HETERO_STD_FRAC * SLAB_DENSITY,
                                  sigma_cells = HETERO_SIGMA_CELLS)
density_grav = copy(truth.density) .+ hetero
grav_model = SyntheticModel(truth.grid, truth.log_rho, density_grav)
@printf("density heterogeneity: std %.2f kg/m3 (%.1f%% of slab contrast), range %.1f to %.1f\n",
        std(hetero), 100 * std(hetero) / SLAB_DENSITY, minimum(hetero), maximum(hetero))
...
gravity = synth_gravity(grav_model, zeros(length(grav_x)), grav_x;
...
config = TrainConfig(
    ...
    weights = LossWeights(gravity = 1.0, mt = 1.0, smooth = 10.0, sigma = 0.1, reference = 0.1),
...
a_ols = t̄ - b_ols * m̄
perturbed_rmse = rmse(truth.log_rho, bundle.mu)
perturbed_corr = anomaly_correlation(truth.log_rho, bundle.mu)
@printf("prior OLS truth = a + b·μ:  a = %+.4f   b = %+.4f   RMSE = %.4f   corr = %.4f\n",
        a_ols, b_ols, perturbed_rmse, perturbed_corr)
...
bc = BoundedCore(lo, hi, 1:1, 1:size(grid, 2), 1:size(grid, 3))
r = bound_report(bc; reference = inv_config.log_bounds)
...
#---------- 9. misspecification numbers ----------

println()
@printf("misspecified gravity:  RMSE %.4f   corr %.4f\n", perturbed_rmse, perturbed_corr)
@printf("unperturbed (ref):     RMSE %.4f   corr %.4f\n", REF_RMSE, REF_CORR)
@printf("delta (this − ref):    RMSE %+.4f   corr %+.4f\n",
        perturbed_rmse - REF_RMSE, perturbed_corr - REF_CORR)
