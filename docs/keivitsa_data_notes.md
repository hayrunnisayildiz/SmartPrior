...
The Cloncurry covariate named `depth` (`DepthCovariate`) is height above the bottom of the bounds box, not depth below the ground. It sets `d = z − z_datum` with `z_datum = z_min`, and `z` is elevation, positive up, so `log_depth` increases toward the top of the box. `depth_below_top` is the same orientation (`cz − z[1]`), a leftover of the z-down grid. Both formulas are unchanged so the Cloncurry feasibility results stay reproducible. Keivitsa uses `depth_below_surface` instead: `d = z_surface(x, y) − z`, positive downward. `z_surface` is the inverse-distance weighting of collar elevations with a finite, non-zero `Z`, frozen when the covariate is built. The network channel uses `log10(max(d, d0) / d0)`. For the Cu feasibility run, `sites/keivitsa.toml` sets **`d0 = 2.5 m`** (half of a 5 m block), fixed before that run. The earlier placeholder **`d0 = 25 m`** (half of Ernest Henry’s 50 m reference thickness) was inherited from Cloncurry; on kept 511P Cu samples about 16% lay shallower than 25 m below the IDW surface, so many `log_depth` values were clamped at the reference.

## Neural training freeze (Keivitsa Cu feasibility)

Synthetic-only diagnosis (`examples/diagnose_nn_keivitsa.jl`, 3-fold grouped CV, semi-synthetic target) compared early-stopping rules before any real-Cu evaluation. **E0** (literal pick: Gaussian NLL loss, early stop on validation NLL, five seeds) was the default aligned with Cloncurry feasibility. On synthetic test skill it reached **0.258** vs kriging **0.331**; early stopping at a **median of 39** steps left train skill near **0.46** — the **validation-NLL stopping criterion** was the problem (H2 in the broader sense: NLL early stop underfits μ).

**E2** (same NLL loss, early stop on **validation RMSE of μ**, five seeds) reached synthetic **validation skill 0.519** vs E0 **0.205**, and test skill **0.411** vs **0.258**, still before real Cu was evaluated. The Keivitsa Cu feasibility run therefore uses **E2**, not E0. This deviation is recorded here so the synthetic diagnosis table and the production run stay linked.
